...
/**
 * Tests when there are two one-time alarms.
 */
public class GlobalManager1NextAlarm6TwoOneTimeAlarmsTest extends FixedTimeTest {
    ...
    private void setAlarm2ToToday() {
        Calendar date = new GregorianCalendar(DayTest.YEAR, DayTest.MONTH, DayTest.DAY, DayTest.HOUR + SHIFT, DayTest.MINUTE + SHIFT);

        OneTimeAlarm oneTimeAlarm = new OneTimeAlarm();
        oneTimeAlarm.setDate(date);
        oneTimeAlarm.setHour(DayTest.HOUR_DAY + SHIFT);
        oneTimeAlarm.setMinute(DayTest.MINUTE_DAY + SHIFT);
    ...
    @Test
    public void t31_justAfter1stAlarm() {
        Calendar date = new GregorianCalendar(DayTest.YEAR, DayTest.MONTH, DayTest.DAY, DayTest.HOUR_DAY, DayTest.MINUTE_DAY);
        ...
        assertThat("Month", nextAlarm.get(Calendar.MONTH), is(DayTest.MONTH));
        assertThat("Date", nextAlarm.get(Calendar.DAY_OF_MONTH), is(DayTest.DAY));
        assertThat("Hour", nextAlarm.get(Calendar.HOUR_OF_DAY), is(DayTest.HOUR_DAY + SHIFT));
        assertThat("Minute", nextAlarm.get(Calendar.MINUTE), is(DayTest.MINUTE_DAY + SHIFT));
    }

    @Test
    public void t32_justBefore2ndAlarm() {
        Calendar date = new GregorianCalendar(DayTest.YEAR, DayTest.MONTH, DayTest.DAY, DayTest.HOUR_DAY + SHIFT, DayTest.MINUTE_DAY + SHIFT);
        date.add(Calendar.SECOND, -1); // 1 second before alarm
        FixedClock clock = new FixedClock(date);
    ...
    @Test
    public void t31_justAfter2nsAlarm() {
        Calendar date = new GregorianCalendar(DayTest.YEAR, DayTest.MONTH, DayTest.DAY, DayTest.HOUR_DAY + SHIFT, DayTest.MINUTE_DAY + SHIFT);
        date.add(Calendar.SECOND, 1); // 1 second after alarm
        FixedClock clock = new FixedClock(date);
